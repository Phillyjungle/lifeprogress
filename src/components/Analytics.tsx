  ...
  return (
    <div className="py-6 px-4">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
        <h1 className="text-2xl font-bold text-white mb-4 md:mb-0">Analytics & Insights</h1>
        
        <div className="flex items-center space-x-4">
          {/* Chart type selector */}
          <div className="flex items-center bg-gray-800 rounded-lg p-1">
            <button 
              className={`p-2 rounded-md ${chartType === 'line' ? 'bg-gray-700 text-white' : 'text-gray-400 hover:text-white'}`}
              onClick={() => setChartType('line')}
            >
              <LineChart className="w-5 h-5" />
            </button>
            <button 
              className={`p-2 rounded-md ${chartType === 'bar' ? 'bg-gray-700 text-white' : 'text-gray-400 hover:text-white'}`}
              onClick={() => setChartType('bar')}
            >
              <BarChart2 className="w-5 h-5" />
            </button>
            <button 
              className={`p-2 rounded-md ${chartType === 'radar' ? 'bg-gray-700 text-white' : 'text-gray-400 hover:text-white'}`}
          ...
          <div className="relative">
            <button 
              className="flex items-center px-4 py-2 bg-gray-800 rounded-lg text-white"
                ...
                {['week', 'month', 'quarter', 'year'].map(option => (
                  <button
                    ...
                    className={`block w-full text-left px-4 py-2 hover:bg-gray-700 ${
                      period === option ? 'text-blue-400' : 'text-white'
                    }`}
                    onClick={() => {
                      setPeriod(option);
      ...
      <div className="bg-gray-800 rounded-lg p-4 mb-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4">
          <h2 className="text-lg font-medium text-white mb-2 md:mb-0">Domain Progress Trends</h2>
          
          <div className="flex flex-wrap items-center gap-2">
            <button
              className={`px-3 py-1 text-xs rounded-full ${
                selectedDomain === 'all' ? 'bg-gray-600 text-white' : 'bg-gray-700 text-gray-300'
              }`}
          ...
          {isChartLoading ? (
            ...
            <ResponsiveContainer width="100%" height="100%">
              ...
              {chartType === 'bar' && (
                ...
                >
                  <CartesianGrid strokeDasharray="3 3" stroke="#444" />
                  <XAxis dataKey="week" tick={{ fill: '#aaa' }} stroke="#555" />
                  <YAxis domain={[0, 10]} tick={{ fill: '#aaa' }} stroke="#555" />
                  <Tooltip 
                    formatter={(value) => [value.toFixed(1), '']}
                    contentStyle={{ backgroundColor: '#333', border: 'none' }}
                  />
              ...
              {chartType === 'radar' && (
                ...
                >
                  ...
                  <PolarAngleAxis dataKey="domain" tick={{ fill: '#aaa' }} />
                  {Object.entries(DOMAIN_CONFIG).map(([key, config]) => (
                    (selectedDomain === 'all' || selectedDomain === key) && (
                      <Radar
                        key={key}
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-8">
        {Object.entries(DOMAIN_CONFIG).map(([domain, config]) => {
          ...
          return (
            <div 
              key={domain}
              className={`bg-gray-800 rounded-lg p-4 border border-transparent
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <div className="bg-gray-800 rounded-lg p-4">
          <h3 className="text-md font-medium text-white mb-4">Current Domain Balance</h3>
        ...
        {/* Weekly Comparison */}
        <div className="bg-gray-800 rounded-lg p-4">
          <h3 className="text-md font-medium text-white mb-4">Weekly Progress</h3>
          <div className="h-[250px]">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart
                data={weeklyAverages}
      ...
      <div className="bg-gray-800 rounded-lg p-4 mb-8">
        <h3 className="text-lg font-medium text-white mb-4">Pattern Analysis</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {Object.entries(analysisResults.variabilityInsights).map(([domain, insight]) => {
            ...
            const statusColor = 
              insight.level === 'high' ? 'bg-red-900 text-red-300' :
              insight.level === 'moderate' ? 'bg-amber-900 text-amber-300' :
              'bg-green-900 text-green-300';
            
            return (
              <div key={domain} className="bg-gray-700 rounded-lg p-3">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center">
                    <div className="rounded-md p-1" style={{ backgroundColor: config?.iconBgColor }}>
                      <Icon className="w-4 h-4" style={{ color: config?.color }} />
                    </div>
                    <h4 className="ml-2 text-sm font-medium text-white">{config?.label}</h4>
                  </div>
                ...
                <p className="text-xs text-gray-300 mt-2">{insight.insight}</p>
              </div>
            );
          })}
        </div>
        ...
        {correlationData.length > 0 && (
          <div className="mt-6">
            <h4 className="text-md font-medium text-white mb-3">Domain Relationships</h4>
                ...
                return (
                  <div key={i} className="bg-gray-700 rounded-lg p-3">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className={`text-xs px-2 py-1 rounded-full ${corr.positive ? 'bg-green-900 text-green-300' : 'bg-red-900 text-red-300'}`}>
